...
function requestAdmins(callback: (response: PersonResponse<Admin>) => void) {
  callback({
    status: "success",
    data: admins,
  });
...
function requestUsers(callback: (response: PersonResponse<User>) => void) {
  callback({
    status: "success",
    data: users,
  });
...
function requestCurrentServerTime(
  callback: (response: { status: string; data: number; error?: string }) => void
) {
  callback({
    status: "success",
    data: Date.now(),
...
function requestCoffeeMachineQueueLength(
  callback: (response: { status: string; data?: string; error: string }) => void
) {
  callback({
    status: "error",
